...
def acc(pred, label):
    ind_pred = pred[label != -1]
    ind_label = label[label != -1]
    return np.sum(ind_pred == ind_label) / len(ind_label)

def fpr_recall(conf, label, tpr_threshold=0.95):
    # 构造二值标签：OOD为0，ID为1（或反过来，根据 ROC_curve 使用时取负值）
    gt = np.ones_like(label)
    gt[label == -1] = 0
    fpr_list, tpr_list, thresholds = metrics.roc_curve(gt, -conf)
    idx = np.argmax(tpr_list >= tpr_threshold)
    return fpr_list[idx], thresholds[idx]

def auc_and_fpr_recall(conf, label, tpr_threshold=0.95):
    # 此处把 OOD 标记为1，ID为0
    ood_indicator = np.zeros_like(label)
    ood_indicator[label == -1] = 1
    fpr_list, tpr_list, thresholds = metrics.roc_curve(ood_indicator, -conf)
    ...
    fpr = fpr_list[idx]
    auroc = metrics.auc(fpr_list, tpr_list)
    precision_in, recall_in, _ = metrics.precision_recall_curve(1 - ood_indicator, conf)
...
def compute_all_metrics(conf, label, pred):
    ...
    auroc, aupr_in, aupr_out, fpr = auc_and_fpr_recall(conf, label, recall_threshold)
    accuracy = acc(pred, label)
    # 返回 [FPR@95, AUROC, AUPR_in, AUPR_out, ID Accuracy]
...
def detection(ind_confidences, ood_confidences, n_iter=100000, return_data=False):
    ...
    all_errors = []
    for delta in np.arange(start, end, gap):
        tpr = np.sum(X1 < delta) / float(len(X1))
        error2 = np.sum(Y1 > delta) / float(len(Y1))
        detection_error = (tpr + error2) / 2.0
        if return_data:
            all_thresholds.append(delta)
            all_errors.append(detection_error)
