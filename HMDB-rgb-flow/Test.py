...
def validate_one_step(model, clip, labels, flow, model_flow):
    ...
    with torch.no_grad():
        ...
        v_predict, v_emd, _, v_emb_masked = model.module.cls_head(v_feat)
        # import pdb;pdb.set_trace()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ood_dataset", type=str, default='Kinetics')  # HMDB UCF Kinetics HAC
    ...
    if args.far_ood:
        if args.ood_dataset == "HAC":
            ...
            eval_dataset = EPICDOMAIN(split='eval', cfg=cfg, cfg_flow=cfg_flow, datapath=args.datapath,
                                      dataset=args.dataset, near_ood=args.near_ood, far_ood=args.far_ood,
                                      ood_dataset=args.ood_dataset)
        eval_dataloader = torch.utils.data.DataLoader(eval_dataset, batch_size=args.bsz, num_workers=args.num_workers,
                                                      shuffle=False,
        ...
        train_dataset = EPICDOMAIN(split='train', eval=True, cfg=cfg, cfg_flow=cfg_flow, datapath=args.datapath,
                                   dataset=args.dataset, near_ood=args.near_ood)
        train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=args.bsz, num_workers=args.num_workers,
                                                       shuffle=False,
                                                       pin_memory=(device.type == "cuda"), drop_last=False)
        ...
        val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=args.bsz, num_workers=args.num_workers,
                                                     shuffle=False,
                                                     pin_memory=(device.type == "cuda"), drop_last=False)
        test_dataset = EPICDOMAIN(split='test', cfg=cfg, cfg_flow=cfg_flow, datapath=args.datapath,
                                  dataset=args.dataset, near_ood=args.near_ood)
        test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=args.bsz, num_workers=args.num_workers,
                                                      shuffle=False,
                                                      pin_memory=(device.type == "cuda"), drop_last=False)
        ...
        if args.near_ood:
            eval_dataset = EPICDOMAIN(split='eval', cfg=cfg, cfg_flow=cfg_flow, datapath=args.datapath,
                                      dataset=args.dataset, near_ood=args.near_ood)
            eval_dataloader = torch.utils.data.DataLoader(eval_dataset, batch_size=args.bsz,
